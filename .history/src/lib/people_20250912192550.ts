...
export async function createPerson(personData: Omit<Person, 'id'>): Promise<Person> {
  ...
  const person: Person = {
    ...
    baptismDate: personData.baptismDate || null,
    confirmationDate: personData.confirmationDate || null,
    marriageDate: personData.marriageDate || null,
    marriagePlace: personData.marriagePlace?.trim() || null,
    divorceDate: personData.divorceDate || null,
    ...
    const results = await runQuery<Person>(
      `MATCH (p:Person) 
       RETURN p.id AS id, 
              ...
              p.baptismDate AS baptismDate,
              p.confirmationDate AS confirmationDate,
              p.marriageDate AS marriageDate,
              p.marriagePlace AS marriagePlace,
              p.divorceDate AS divorceDate,
