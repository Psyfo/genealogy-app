...
export async function addParentChildRelationship(childId: string, parentId: string, relationshipType: 'father' | 'mother'): Promise<void> {
  // Validate IDs
...
export async function removeParentChildRelationship(childId: string, parentId: string): Promise<void> {
  // Validate IDs
  const childValidation = validateId(childId);
  const parentValidation = validateId(parentId);
