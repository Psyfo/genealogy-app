...
import { getAllPeople, addParentChildRelationship, removeParentChildRelationship, getFamilyMembers } from "@/lib/people";
...
export default function RelationshipManager({ person, onClose, onUpdate }: RelationshipManagerProps) {
  ...
  const loadData = async () => {
    try {
      ...
      const [people, family] = await Promise.all([
        getAllPeople(),
        getFamilyMembers(person.id)
      ]);
      
      setAllPeople(people.filter(p => p.id !== person.id));
  ...
  const handleAddRelationship = async (parentId: string, relationshipType: 'father' | 'mother') => {
    try {
      await addParentChildRelationship(person.id, parentId, relationshipType);
      await loadData();
      onUpdate();
    } catch (err) {
  ...
  const handleRemoveRelationship = async (parentId: string) => {
    try {
      await removeParentChildRelationship(person.id, parentId);
      await loadData();
      onUpdate();
    } catch (err) {
